...
@Component({
  selector: 'app-tabela-baixa',
  templateUrl: './tabela-baixa.component.html',
  styles: [
  ]
})
export class TabelaBaixaComponent implements OnInit {
  ...
  constructor(private store: StorageService, private auth: AuthService, private printer: ServicePrintMove) {
    ...
    this.user = auth.user;
    this.page = new StorageServicePaginateService(this.store, this.auth, ServiceBaixa.STORAGE_NAME, 'id')
  }
  async ngOnInit() {
    this.listMove = await new ServiceBaixa(this.store).findMovType("DOWNLOAD");
  ...
  printMov(attr: any) {
    let move: ServiceBaixa = new ServiceBaixa(this.store)
    move.oItem = attr
